...
class Server:
    ...
    def _get_avg(self, card, chan):
        with self.lock:
            ...
            self.trigger(False)
            time.sleep(0.1)
            data = self.get_data(card, chan, False) # Get the data associated with the card & channel,
						    # Stores it into an array of unsigned 16-bit integers. 
            return data.mean() # Get the mean (average) of the elements in the array "data"
        ...
        with self.lock:
            for card in self.cards:
                for chan in range(8):
                    ...
                    offset = value
                    self._set_offset(card, chan, offset) # Set first offset
                    avg = self._get_avg(card, chan) # Average of the data points (as unsigned 16-bit integers) of the channel & card
		
		    # Every iteration add the difference between "value" & "avg" to offset, and stopp setting the offset once this value exceeds "delta".
